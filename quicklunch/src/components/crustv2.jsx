...
export default function CrustV2(){
    const [foodOps, setFoodOps] = useState([
        {"name": "Classic Hand Tossed", "img": Classic, "description": "Embrace tradition with our Classic Hand-Tossed crust. It strikes a delightful balance in thickness, offering a texture that's crispy on the outside yet soft inside. Perfect for those who value classic flavors in every bite", "glutenFree": false, "keto": false, "new":false}, {"name": "Thin & Crispy", "img": Thin, "description": "Our Thin & Crispy crust is a haven for crunch enthusiasts. Rolled to fine thinness, it delivers a satisfyingly crisp texture, making it ideal for those who love a lighter base to highlight their toppings.", "glutenFree": false, "keto": false, "new":false}, {"name": "Deep Dish", "img": Deepdish, "description": "Dive into the richness of our Deep Dish crust. Its thick, buttery texture is akin to biting into a cloud of savory delight. A fantastic choice for anyone seeking a more filling and robust pizza.", "glutenFree": false, "keto": false, "new":false},
    ...
    const handleSetCrust = (pizzaIndex, crustType) => {
        dispatch(setCrust({ pizzaIndex, crust: crustType }));
    };
    ...
    return(
        <div className='flex flex-row'>
            ...
            <li className="step">Sauce</li>
            <li className="step">Toppings</li>
            <li className="step">Additional</li>
            </ul>
            <h1 className='font-bold text-xl mb-5'>Crust</h1>
           <p className='text-xl mb-10'>Choose your crust. We offer a variety of options to suit your dietary needs and preferences. <span className='font-bold'>Select only one option below.</span></p>
           ...
           {foodOps.map(item => (
            ...
            <div className="card-body">
                ...
                <button className="btn btn-primary" onClick={() => {handleSetCrust(selectPizza,item.name)}}>Select</button>
            </div>
            </div>))}
            </div>
        </div>
        ...
        <button className='btn btn-secondary w-2/3 self-center my-5' onClick={()=>handleNewPizza()}>Create New Pizza</button>
        {pizzaOrder && (
                <div>
